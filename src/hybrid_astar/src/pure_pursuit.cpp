...
void pure_pursuit() {
	...
	x = transform_robot.getOrigin().x() - DELTAR * cos(yaw);
	y = transform_robot.getOrigin().y() - DELTAR * sin(yaw);
	...
	while(true) {
		...
		if(dirs[index] == -1) {
			...
			float m = hypot(x_t - path.poses[0].pose.position.x, y_t - path.poses[0].pose.position.y);
			for (int i = index; i < index + 4; ++i) {
				if(hypot(x_t - path.poses[i].pose.position.x, y_t - path.poses[i].pose.position.y) < m) {
			...
			for (int i = index; i < index + 4; ++i) {
				if(hypot(x_t - path.poses[i].pose.position.x, y_t - path.poses[i].pose.position.y) > lf) {
					index = i;
					break;
				}
			}
			...
			if(hypot(x_t - path.poses[n-1].pose.position.x, y_t - path.poses[n-1].pose.position.y) < 0.2) {
				ROS_INFO("GOAL REACHED!");
				break;
			}
